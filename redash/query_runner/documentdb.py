...
class DocumentDBJSONEncoder(JSONEncoder):
    def default(self, o):
        if isinstance(o, ObjectId):
            return str(o)
        elif isinstance(o, Timestamp):
            return super(DocumentDBJSONEncoder, self).default(o.as_datetime())
        elif isinstance(o, Decimal128):
            return o.to_decimal()
        return super(DocumentDBJSONEncoder, self).default(o)
...
class DocumentDB(BaseQueryRunner):
    ...
    def __init__(self, configuration):
        super(DocumentDB, self).__init__(configuration)
    ...
    def test_connection(self):
        ...
        if not db.command("connectionStatus")["ok"]:
            raise Exception("DocumentDB connection error")
    ...
    def run_query(self, query, user):
        ...
        logger.debug(
            "DocumentDB connection string: %s", self.configuration["connectionString"]
        )
        logger.debug("DocumentDB got query: %s", query)
        ...
        error = None
        json_data = json_dumps(data, cls=DocumentDBJSONEncoder)
